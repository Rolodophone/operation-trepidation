...
class Level(val state: StateGame, val title: String, val musicIndex: Int, val gauges: FloatArray, val gaugeSpeeds: FloatArray, val clearCondition: () -> Boolean) {
    ...
    init {
        state.ctx.music.prepMusic(musicIndex)
    }
    ...
    fun update() {
        ...
        if (currentTime - startTime > 4000) { //update actual level
            //update gauges
        ...
        if (currentTime - startTime > 6000f) state.ctx.music.resume()
